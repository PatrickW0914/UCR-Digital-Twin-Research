...
model Circuit_3A_S3
  ...
  // buildings (HUB, Campus Surge, Physics). 
  // Single 1 MWh BESS sited at HUB-B 480 V LV bus, controlled on T3 substation transformer % loading.
  ...
  //
  // All PV in non-export mode which means no backflow to MV.
  ...
  // S3 reported outputs.
  // RealExpression source blocks feed each output connector via connect()
